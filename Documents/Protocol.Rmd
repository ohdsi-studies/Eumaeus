...
abbreviations <- readr::read_delim(col_names = FALSE, delim = ";", trim_ws = TRUE, file = "
  ...
  PMaxSPRT; Poisson-based MAXimized Sequential Probability Ratio Test 
  PS; Propensity score
...
- Sensitivity and specificity based on the PMaxSPRT decision rule
- Timeliness: measure of the time it takes for a method the identify safety signals 
...
# NOTE: we need to reference up before sending for registration
...
- Cohort method using a contemporary non-user comparator. We require the non-user comparator to have an outpatient visit on the index date, and have no vaccinations for the same disease as the target vaccine during the study period.
    - Crude comparison. The comparator cohort will be a random sample (of outpatient visits) of equal size as the target cohort.
    - Using 1-on-1 propensity score (PS) matching. The comparator cohort before PS matching will be a stratified (by age and sex) random sample of four times the size as the target cohort. (two times for the Seasonal Flu Vaccination (all) target cohort for computational reasons).
...
- Case-control
    ...
    - By sampling controls from the general non-case population, and adjusting for age and sex in the outcome model.
