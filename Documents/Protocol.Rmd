...
**MAS** receives grant funding from the US National Institutes of Health and the US Food & Drug Administration and contracts from the US Department of Veterans Affairs, Janssen Research and Development and IQVIA.
...
* Exposures: previous viral vaccines including 2017-2018 flu, H1N1 flu, Human Papillomavirus (HPV), and Varicella-Zoster.
* Outcomes: selected adverse events of special interest; negative control outcomes; synthetic positive control outcomes
* Analyses: 
...
* Metrics:
    ...
    - Detection time: the number of months until 80% of all positive controls exceeds the critical value
...
The results of this evaluation will help us understand how these methods will perform when applied to COVID vaccines. 
...
Manual review of this list by clinical experts created the final list of 100 negative control outcomes. 
...
Target true hazard ratios for the positive control synthesis are 1.5, 2, and 4, so using the 100 negative controls we are able to construct 100 * 3 = 300 positive control outcomes.
...
data_sources <- readr::read_delim(col_names = TRUE, delim = ";", trim_ws = TRUE, file = "
  ...
  Clinical Practice Research Datalink (CPRD) GOLD ; UK, NHS primary care ; 10M ; 1995 -- ; Primary care EHR.
")
...
- Stratifying by age (in 5 year increments) and sex, allowing the expected rate to be adjusted for the demographics of the vaccinated.
...
We define a non-user comparator cohort as having an outpatient visit on the index date, and having no vaccinations for the same disease as the target vaccine during the study period. 
We will evaluate two variants:

- Crude comparison. The comparator cohort will be a random sample (of outpatient visits) of equal size as the target cohort.
- Using 1-on-1 propensity score (PS) matching. The comparator cohort before PS matching will be a stratified (by age and sex) random sample of four times the size as the target cohort. (two times for the Seasonal Flu Vaccination (all) target cohort for computational reasons). Propensity models will use a large generic set of covariates, including demographics and covariates per drug, condition, procedure, measurement, etc., and will be fitted using large-scale regularized regression as described previously. [@Tian2018-xy]
...
We will evaluate four variants:
...
- By sampling controls from the general non-case population, and adjusting for age and sex in the outcome model. The control sample will be four time the number of controls. Age will be modeled as one variable per 5-year age category.
...
In our experiment we will consider one decision rule, which is the critical value computed for the LLR at an alpha of 0.01, using a binomial model. 
...
- 3 $\times$ 100 = 300 positive controls
- 3 dose definitions (both, first, second) for the zoster and HPV vaccines, 1 for H1N1 and seasonal flu.

Resulting in a total of 10 $\times$ 3 $\times$ [(9 + 9 + 9 + 9) $\times$ 1 + (12 + 12) $\times$ 3]  $\times$ (100 + 300) = 1,296,000 effect-size estimates. Each estimate will contain:
...
Clinical Practice Research Datalink (CPRD) GOLD & Independent Scientific Advisory Committee will review the protocol prior to study execution (TODO: change tense to past after review, and add protocol ID).
")
